...
Post-cutover authoritative docs live in the monorepo itself:
- `MEMORY_AND_TRAINING.md` — the deepest doc: memory substrates, per-source training pipelines, retrieval, prompt assembly, mode system (§14 phase log, §17 dated changelog).
- `AUDIO_TRAINING_PLAN.md` + `AUDIO_TRAINING_PLAN_IMPLEMENTATION.md` — audio-training feature plan + build log.
- `AVATAR_MODES_PLAN.md` + `AVATAR_MODES_IMPLEMENTATION.md` — avatar modes plan + build log.
- `docs/autonomous_agent_design.md` — autonomy design: identity modes, capabilities, risk levels, rollout gates.
- `docs/TESTING_PLAN.md` — verification guide for Waves A/B/C. `docs/endpoints.md` — endpoint catalog.
...
- **Auth:** Devise (cookie web) + hand-rolled JWT (API) — see `apps/backend/app/controllers/api/v1/base_controller.rb`. **No devise-jwt.** Mobile social login (Google/Apple/Facebook) is done via client-side provider SDK + server-side token verification; `omniauth` is used for **web flows only** (see the Decisions entry in Phase status).
...
- Phase 7 (archive + cutover) — **✔ complete**. Workspace-level `MIGRATION_COMPLETE.md` records the 5-repo → monorepo mapping, per-phase commit ledger, and first-run checklist. Workspace `CLAUDE.md` carries a banner pointing operators here; the old `gln-*` repos are **preserved in place** for reference (not deleted; archival is an explicit operator action). Final cross-phase smoke test (all green): rails `/up`, fastapi `/health`, JWT login w/ `role: administrator`, chat/proactive/social-status/device-tokens endpoints, web login + `/admin` + `/admin/users` + `/admin/instagram`, FastAPI guard (401 without token / 200 with).

Post-cutover work (chronological, from git history — dates are commit dates):

- **Backlog wave (2026-04-23)** — **✔ complete**. Global + per-user feature flags: `FeatureSetting` / `UserFeatureSetting` (missing row = enabled; `GET/POST /api/v1/settings/features`, admin panel at `/admin/feature_settings`), gating proactive skills via `Proactive::SkillRegistry.enabled_skill_ids_for`. Scheduled proactive pushes: Whenever cron (`config/schedule.rb`) fires `ProactivePushJob` every 30 min per time-of-day window → `ProactiveGreetingForUserJob` (one per user; timezone-aware, throttled) → `PushNotificationJob`. `SyncAvatarCountersJob` nightly at 04:00 (pulls `insights_count` from FastAPI). `JwtDenylist` for token revocation. Migration `20260423000008` **drops `avatars.knowledge_level`** in favour of raw counters (`insights/conversations/messages/social_connections_count`, `days_active`, `last_interaction_at`); `Avatar#knowledge_level` is now a derived log-scale 1–10 method. Extra insight chains (facebook/twitter content, spotify taste); expanded admin users pages (promote/demote/reset_onboarding/extract/disconnect_social).
- **Avatar modes (2026-04-29)** — **✔ complete**. Profesional / Amigos / Citas. `avatars.active_mode` (default `friends`) + `mode_message_counts` jsonb incremented atomically (`jsonb_set`) per user message; `PATCH /api/v1/avatar` switches mode (invalid → 422 with friendly Spanish error); `ChatGenerationJob` sends `active_mode` + `mode_message_count` to FastAPI; the system prompt gains a `{mode_section}` block anchored near the top — Citas ramps nascent (<30 msgs) → warming (<150) → established; persona insights are mode-scoped (`source: "persona_update:<mode>"`; legacy bare `persona_update` rows surface only in friends mode). Expo ModeSwitcher + chat-header chip. Docs: `AVATAR_MODES_PLAN.md` / `AVATAR_MODES_IMPLEMENTATION.md`.
- **Audio training / voice journal (2026-04-29 → 05-02)** — **✔ complete**. Continuous recording → transcription → insights. Rails: `AudioSession` (status machine) / `AudioChunk` (Shrine `AudioChunkUploader`, 25MB cap, AAC/m4a/mp3/wav; idempotent create via `Idempotency-Key` header) / `VoiceEnrollment` (start/stop trigger-phrase audio; voice embedding deferred to Phase 2) / `AudioUsage` (daily quota rollup, row-locked upsert) + `AudioQuota` service; routes under `/api/v1/audio/*` (sessions + finish/cancel, chunks, wipe, voice_enrollment); jobs `AudioChunkProcessJob` (quota check → FastAPI multipart transcribe → insights + events) / `AudioSessionFinalizeJob` / hourly `AudioSessionCleanupJob` (cron). FastAPI: `app/audio/transcription.py` provider abstraction (default `gpt-4o-mini-transcribe`). Expo: center "Grabar" tab. Docs: `AUDIO_TRAINING_PLAN.md` / `AUDIO_TRAINING_PLAN_IMPLEMENTATION.md`.
- **Client hardening (2026-05-01)** — telemetry, self-healing Cable subscriptions, 30 s request timeout, Android safe-area + keyboard fixes, app logo.
- **Memory & training build-out (2026-05-04 → 05-06)** — **✔ complete**. Phases 1–18 plus Waves A/B/C; authoritative record in `MEMORY_AND_TRAINING.md` (§14 phase log, §17 changelog). Highlights: four memory substrates in the AI DB (`insights`, `user_events`, `conversation_summaries`, `audio_transcript_chunks`) with 6 concurrent chat-time retrievals; event extraction from chat/audio/social posts; rolling conversation summaries + history truncation; verbatim transcript RAG; web-search-engagement insights; retrieval distance ceilings + pre-insert dedupe (Wave A); correction detection with `insights.status`, per-category confidence floors, typed `user_style_profiles`, durable post-turn learning (`PostTurnLearningJob` → `POST /internal/chat/learn`), deterministic voice/memory eval harness (Wave B); autonomy scaffolding (Wave C): agent identity prompt policy (companion/representative/draft/autopilot), `AgentPermission` (default-deny per-capability) + `AgentActionLog` (append-only audit), draft-mode `/api/v1/agent/proposals` (+ approve/reject/edit) — **no tool execution wired**, rollout gates 4–7 pending (`docs/autonomous_agent_design.md` §6). Admin memory drill-down at `/admin/users/:id/memories` (Rails proxies FastAPI via `AiAgentsClient`) + `/internal/memory/:user_id/snapshot`. FastAPI unit suite: 131 tests. Verification guide: `docs/TESTING_PLAN.md`.
- **Voice chat (2026-05-07)** — **✔ complete**. Back-and-forth audio inside conversations: `POST /api/v1/chat/conversations/:id/messages/voice` (multipart) → FastAPI transcription → user `Message` persisted with the source audio attached → voice-mode `ChatGenerationJob` (`voice_response: true`) which synthesizes the assistant reply via FastAPI TTS (`app/audio/synthesis.py`, OpenAI `tts-1`, 4096-char input trim) and attaches the mp3 to the assistant `Message`; clients fetch bytes from JWT-gated `GET /api/v1/chat/messages/:id/audio`; Expo `audioRecorder` / `audioUploader` / `audioMessagePlayer` services.
- **Quota + error UX (2026-06-02)** — quota-exhaustion and login errors surface friendly messages end-to-end (`chat_service.py`, `AiAgentsClient`, `Auth::JwtIssuer`, `apiService.js`).

See `../PORT_PLAN.md` §10 for the original port roadmap; post-cutover feature design lives in the monorepo docs listed at the top of this file.
...
**Knowledge level is derived and Rails-canonical (1–10).** The `avatars.knowledge_level` column was dropped (migration `20260423000008`) in favour of raw counters (`insights_count`, `conversations_count`, `messages_count`, `social_connections_count`, `days_active`); `Avatar#knowledge_level` computes a log-scale 1–10 value from them. FastAPI's `UserProfile.knowledge_level` accepts 1–10 as-is — **no scale conversion anywhere** (the old 1–5 halving is gone; the prompt halves for display only). Counters are maintained by `after_create_commit` hooks + nightly `SyncAvatarCountersJob`.
